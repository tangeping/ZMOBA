...
namespace KBEngine
{
    ...
    public class Logger
    {
        ...
        public static void Debug(object message, Boolean isShowStack = SHOW_STACK, string user = "")
        {
            //if (user != 11)
            //    return;
            ...
            if (LogLevel.DEBUG == (CurrentLogLevels & LogLevel.DEBUG))
                Log(string.Concat(" [DEBUG][" + user + "]: ", isShowStack ? GetStackInfo() : "", message), LogLevel.DEBUG,false);
        }
        ...
        private static void Log(string message, LogLevel level, bool writeEditorLog = true)
        {
            ...
            var msg = string.Concat(System.DateTime.Now.ToString("d"), message);
